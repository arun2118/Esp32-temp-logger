...
A robust, production-ready data logging firmware that captures telemetry from **RuuviTag BLE Sensors**. Designed specifically for off-the-grid field applications, this system operates entirely **without internet access or cellular data**, serving an interactive dark-themed dashboard over a local Wi-Fi Access Point (AP).
